\include "_internal/utilities/lilypond-version-predicates.ily"
...
#(define (calcClusterThickness thick)
   "Provide a value for cluster padding for given thickness (in staff spaces).
    Compensate behaviour changes in 2.19.
    Values below 0.5 won't work as intended."
   (if (lilypond-greater-than? "2.18.2")
       (if (= 0 thick) 0.01 (/ thick 2))          ; for both versions, avoid returning zero
       (if (= 1 thick) 0.01 (- (/ thick 2) 0.5))))

% ToDo:
% Do we want to have the following overrides here?

\layout {
  \context {
    \Voice
    \override ClusterSpanner.layer = #-5
    \override ClusterSpanner.X-offset = #0.6
    \override ClusterSpanner.padding = #(calcClusterThickness 1)
  }
}
...
\layout {
  \context {
    \Voice    % if necessary, this should be repeated for CueVoice etc. ...
    \override VoiceFollower.layer = #-2
    \override Glissando.layer = #-2
  }
}
